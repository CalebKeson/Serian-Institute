...
const CashFlow = () => {
  ...
  const handleExport = () => {
    ...
    const headers = ['Period', 'Cash Inflows', 'Cash Outflows', 'Net Cash Flow', 'Closing Balance'];
    const data = cashFlow.cashFlow.map(period => [
      period.period,
      period.inflows,
      period.outflows,
      period.netCashFlow,
    ...
    a.href = url;
    a.download = `cash_flow_${dateRange.startDate}_to_${dateRange.endDate}.csv`;
    a.click();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-600 mb-1">Total Cash Inflows</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-600 mb-1">Total Cash Outflows</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className={`text-2xl font-bold ${summary.totalNetCashFlow >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-2xl font-bold text-blue-600">{formatCurrency(closingBalance)}</p>
          </div>
        </div>
        ...
        <div className="mt-6 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          >
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium text-gray-900">Cash Flow Details by Period</span>
            </div>
            {expandedSections.details ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
          </button>

          {expandedSections.details && cashFlow?.cashFlow && (
            <div className="p-6">
              <div className="space-y-6">
                {cashFlow.cashFlow.map((period, index) => (
                  <div key={index} className="border-b border-gray-200 last:border-b-0 pb-4 last:pb-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {Object.entries(period.inflowBreakdown || {}).map(([source, amount]) => (
                          ...
                          {Object.keys(period.inflowBreakdown || {}).length === 0 && (
                            <div className="text-sm text-gray-500 italic">No cash inflows this period</div>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {Object.entries(period.outflowBreakdown || {}).map(([category, amount]) => (
                            <div key={category} className="flex justify-between items-center">
                              <span className="text-sm text-gray-600">{category}</span>
                              <span className="text-sm font-medium text-red-600">{formatCurrency(amount)}</span>
                            </div>
                          ))}
                          {Object.keys(period.outflowBreakdown || {}).length === 0 && (
                            <div className="text-sm text-gray-500 italic">No cash outflows this period</div>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`text-lg font-bold ${period.netCashFlow >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {formatCurrency(period.netCashFlow)}
                        </span>
                      </div>
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-sm text-gray-500">Closing Balance</span>
                        <span className="text-md font-medium text-blue-600">{formatCurrency(period.closingBalance)}</span>
                      </div>
                    </div>
                  </div>
                ))}
