...
const RecordPayment = () => {
  ...
  const [fetchingEnrollments, setFetchingEnrollments] = useState(false); // ADDED: separate loading state for enrollment fetch
  ...
  useEffect(() => {
    ...
    const preSelectFromUrl = async () => {
      ...
      if (urlStudentId && students.length > 0) {
        ...
        if (student) {
          setFetchingEnrollments(true); // Start enrollment fetch
          try {
            ...
            setFetchingEnrollments(false); // End enrollment fetch
          }
        }
      } 
            ...
            <div className="p-6">
              ...
              {showStudentDropdown && (
                <div className="absolute z-10 w-full max-w-2xl mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                  {fetchingEnrollments ? ( // Show loader inside dropdown while fetching enrollments
